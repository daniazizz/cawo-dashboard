...
function CostInput({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <label className="text-xs text-zinc-500" htmlFor={`cost-${productTitle}`}>
        Unit cost (EUR)
      </label>
      <input
        ...
        className="w-20 rounded border border-zinc-700 bg-zinc-800 px-2 py-1 text-right text-xs tabular-nums text-zinc-100 focus:border-zinc-500 focus:outline-none disabled:bg-zinc-900"
      />
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900 shadow-sm">
      <button
        ...
        className="flex w-full items-center justify-between border-b border-zinc-800 px-5 py-3 text-left"
      >
        <div>
          <h2 className="text-sm font-semibold text-zinc-200">Inventory</h2>
          {warning && (
