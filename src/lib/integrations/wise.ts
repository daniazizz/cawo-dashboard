...
export async function fetchWiseBalances(): Promise<WiseCashBalance[]> {
  ...
  if (!token || !profileId) {
    throw new Error(
      "Missing WISE_API_TOKEN or WISE_PROFILE_ID environment variables",
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Wise API request failed: ${response.status} ${response.statusText} — ${body.slice(0, 300)}`,
    );
  }
...
export async function fetchRatesToEur(
  currencies: string[],
  ...
  const uniqueCurrencies = Array.from(new Set(currencies)).filter(
    (c) => c !== "EUR",
  );
  ...
  await Promise.all(
    uniqueCurrencies.map(async (currency) => {
      const response = await fetch(
        `${WISE_API_BASE}/v1/rates?source=${currency}&target=EUR`,
        {
